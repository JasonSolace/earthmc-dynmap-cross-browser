...
function parseArgs(argv) {
	...
	try {
		validateProjectionOptions(opts, ["inputPath", "outputPath", "nameProperty"]);
	} catch (error) {
		fail(error.message);
	}
...
function clipRingToBounds(ring, bounds) {
	...
	points = clipPolygon(
		points,
		(point) => point[0] >= bounds.lonMin,
		(a, b) => intersectVertical(a, b, bounds.lonMin),
	);
	points = clipPolygon(
		points,
		(point) => point[0] <= bounds.lonMax,
		(a, b) => intersectVertical(a, b, bounds.lonMax),
	);
	points = clipPolygon(
		points,
		(point) => point[1] >= bounds.latMin,
		(a, b) => intersectHorizontal(a, b, bounds.latMin),
	);
	points = clipPolygon(
		points,
		(point) => point[1] <= bounds.latMax,
		(a, b) => intersectHorizontal(a, b, bounds.latMax),
	);
...
function scaleProjectedPoint(point, options, projectedBounds) {
	const xRatio =
		(point.x - projectedBounds.xMin) /
		(projectedBounds.xMax - projectedBounds.xMin);
	const yRatio =
		(projectedBounds.yMax - point.y) /
		(projectedBounds.yMax - projectedBounds.yMin);
...
function simplifyClosedRing(points, tolerance) {
	...
	if (
		!samePoint([closed[0].x, closed[0].z], [closed.at(-1).x, closed.at(-1).z])
	) {
...
function main() {
	...
	for (const feature of geojson.features || []) {
		const countryName = findCountryName(
			feature?.properties,
	...
	console.log(
		`Converted ${keptCountries} countries across ${keptRings} polygon rings.`,
	);
